...
export default async function fetchDevMessages(session:string, setMessages:Dispatch<SetStateAction<never[]>>, callback?:()=>void){
    ...
    const url = "/api/messages/dev?session="+session + "&n=" + initMessageNum
    // console.log(url)
    let res:Response;
    res = await fetch(url);
    for(let n =0;n<10;n++){
