...
export default async function NewsMessageSubmittedHandler(e:FormEvent<HTMLFormElement>, session:string, callbackRef:MutableRefObject<()=>void>){
    e.preventDefault()
    ...
    let res = {ok:false};
    for(let n =0;n<10;n++){
    res = await fetch("/api/messages/news/send", {
        method: "POST",
        body:JSON.stringify({
            session:session,
            content:value
